...
class Host:
   ...
   def stopVgcDriver(self):
       ...
       # bug in ubuntu, need to sleep before unloading vgcd
       time.sleep(120)
   ...
   def cat_etc_issue(self):
	  ...
	  trace_info("Coundn't find the version from /etc/issue, trying from kernel")
	  ...
	  if re.search("2\.6\.32",kernel):
		  ...
		  return "redhat5"
	  elif re.search("el7",kernel):
		  trace_info("Seem like redhat7 kernel")
		  return "redhat7"
	  elif re.search("3\.13\.0-",kernel):
		  trace_info("seem like ubuntu14.04 kernel")
		  return "ubuntu1404"
	  else:
   ...
   def installBuild(self,build,stats = 0,server = "cloudy.virident.info", toolsTest = 1,pathType = "releases",emc = 0,firmware = "0",releaseVer = None,rdma = None):
	...
	if linux == "redhat6":
		...
		found_redhat = "1"
		prependstr = self.run_command_get_output("uname -r")[1] + '-' 
   ...
   def is_directory_empty(self,directory ):
      ...
      count = self.run_command_single_output("ls -a -I 'kexec*' %s | wc -l"%directory)
      count = int(count[0])
