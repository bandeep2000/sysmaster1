...
class Tests:
   ...
   def __init__(self,host,testcaseStr,database = 0):
   ...
   def _set_initial_conf_str(self,device,string = ""):
       ...
       details = self.host.vgcconfig.getDevicesDetails("-d %s"%device)[device]
   ...
   def configure(self,device,mode = None,n = None,tbr = None):
       ...
       if mode == "reset":
           ...
           self._set_initial_conf_str(device,vgcConfig,string = "Reset")
   ...
   def installBuild(self,build,stats = 0,server = "cloudy.virident.info",pathType = "packages",emc  = 0):

       self.host.installBuild(build,stats,server,pathType = pathType, emc = emc)
       self.build = build
	...
	for t in self.testcases:
		...
		if tName == testcaseName:
			...
			t.reasonPassed = reasonFailed
                        ...
                        return 1
	#print_green("=" * 80)
	#trace_success("TESTCASE: Failed %s,reason '%s'"%(testcaseName,reasonFailed))
	#print_green("=" * 80)
   ...
   def runVdbenchMultiple(self,dict):
       ...
       rc = vb.runCreatePartitions(dict)
   ...
   def runFio(self,
       ...
       parsedOut,rc,wrtAmp = fio.run(devPart,bs,time,rw,ioengine,numjobs,rw_perc,ba,fsync,size,iodepth,fill_drive,verify)
       ...
       self.print_tcase_success(testcaseName,"RW card attributes changed, no error in logs, output = '%s',wrtAmp='%s'"%(parsedOut,wrtAmp))
